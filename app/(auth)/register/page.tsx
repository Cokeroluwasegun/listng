...
export default function RegisterPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-[color-mix(in_srgb,var(--muted),transparent 40%)] px-4 py-12">
      <div className="w-full max-w-lg">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="font-display text-3xl font-extrabold text-[var(--foreground)]">
            Create your account
          </h1>
        ...
        <button
          ...
          className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--color-primary)] py-3.5 font-semibold text-white shadow-lg shadow-[color-mix(in_srgb,var(--color-primary),transparent 25%)] transition-all hover:opacity-90 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-40"
        >
          Continue
        ...
        {/* Trust indicators */}
        <div className="mt-8 flex items-center justify-center gap-6 border-t border-[var(--border)] pt-6">
  ...
  return (
    <button
      ...
      className={`w-full rounded-xl border-2 p-5 text-left transition-all duration-200 ${
        selected
          ? "border-[var(--color-primary)] bg-[color-mix(in_srgb,var(--color-primary),transparent 4%)] shadow-md"
          : "border-[var(--border)] bg-white hover:border-[color-mix(in_srgb,var(--color-primary),transparent 40%)] hover:shadow-sm"
      }`}
    >
      <div className="flex items-start gap-4">
        <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl ${
          selected ? "bg-[var(--color-primary)]" : "bg-[var(--muted)]"
        } transition-colors`}>
          <Icon className={`h-6 w-6 ${selected ? "text-white" : "text-[var(--muted-foreground)]"}`} />
        </div>
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <h3 className="font-display font-bold text-[var(--foreground)]">{title}</h3>
          ...
          <ul className="mt-3 space-y-1.5">
            {features.map((f) => (
              <li key={f} className="flex items-center gap-2 text-sm text-[var(--foreground)]">
                <CheckCircle className={`h-3.5 w-3.5 shrink-0 ${selected ? "text-[var(--color-primary)]" : "text-green-500"}`} />
                {f}
              </li>
            ))}
          </ul>
